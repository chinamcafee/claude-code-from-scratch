# 第 11-2 节：补上历史管理、成本预算和会话恢复

这一小节会把 `src/agent.ts` 升级到第二阶段：虽然还没有完整的 runtime，但已经有了更像“真正 Agent 对象”的公共操作。
...
agent.restoreSession({ anthropicMessages: [{ role: "user", content: "hello" }] });
agent.showCost();
agent.clearHistory();
...
下一小节替换成最终版 runtime：[11-agent-03-full-runtime.md](./11-agent-03-full-runtime.md)
